...
void refine(int ts)
{
   int i, j, n, in, num_refine_step, nm_t = 0;
   // int num_split;
   double t0 = timer(), t1, t2, t4, t5;
   block *bp;
   ...
   for (i = 0; i < num_refine_step; i++) {
      ...
      t2 = timer();
      split_blocks(); // tasks here!
      t5 = timer();
      timer_refine_sb += t5 - t2;
      t4 += t5 - t2;

      t2 = timer();
      consolidate_blocks(); // tasks here!
      t5 = timer();
      timer_cb_all += t5 - t2;
      t4 += t5 - t2;
   }
   ...
   timer_refine_all += timer() - t0;
}

int refine_level()
{
   int level, nei, n, i, j, b, c, c1, change, p, in;
   block *bp;
   parent *pp;
