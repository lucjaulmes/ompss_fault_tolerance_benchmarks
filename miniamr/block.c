...
void split_blocks()
{
   ...
   int i, j, k, m, n, o, nl, xp, yp, zp, c, c1, other, fcase, p,
       i1, j1, k1, level, sib[8];
   block *bp, *bp1;
   parent *pp;
         ...
         if (blocks[n].number >= 0 && blocks[n].level == level) {
            ...
            if (bp->refine == 1) {
               ...
               for (o = 0; o < 8; o++) {
                  ...
                  box_arr_t array_from = (box_arr_t)bp->array;
                  box_arr_t array_to = (box_arr_t)bp1->array;
                  ...
                  for (int start = 0; start < num_vars; start += comm_vars) {
                    ...
                    #pragma omp task label(split_block) in(array_from[start;number]) out(array_to[start;number]) \
                        firstprivate(x_block_size, y_block_size, z_block_size, start, number, i1, j1, k1) default(none)
                    {
                      for (int v = start; v < start+number; v++) {
                        split_block_task(x_block_size+2, y_block_size+2, z_block_size+2, array_from[v], array_to[v], i1, j1, k1);
                      }
                    }
                  }
               }
...
void consolidate_blocks()
{
   static int side[6][2][2] = { { {0, 2}, {4, 6} }, { {1, 3}, {5, 7} },
                                { {0, 1}, {4, 5} }, { {2, 3}, {6, 7} },
                                { {0, 1}, {2, 3} }, { {4, 5}, {6, 7} } };
   int n, p, i, j, k, i1, j1, k1, level, o, c, other, c1, fcase;
   block *bp, *bp1;
   parent *pp;
      ...
      for (p = 0; p < max_active_parent; p++)
         if ((pp = &parents[p])->number >= 0 && pp->level == level &&
            ...
            for (o = 0; o < 8; o++) {
               ...
               typedef double (*box_arr_t)[x_block_size+2][y_block_size+2][z_block_size+2];
               box_arr_t array_from = (box_arr_t)bp1->array;
               box_arr_t array_to = (box_arr_t)bp->array;
               ...
               for (int start = 0; start < num_vars; start += comm_vars) {
                 ...
                 #pragma omp task label(consolidate_block) in(array_from[start;number]) out(array_to[start;number]) \
                     firstprivate(x_block_size, y_block_size, z_block_size, start, number, i1, j1, k1) default(none)
                 for (int v = start; v < start+number; v++)
                     consolidate_block_task(x_block_size+2, y_block_size+2, z_block_size+2, array_from[v], array_to[v], i1, j1, k1);
               }
            }
